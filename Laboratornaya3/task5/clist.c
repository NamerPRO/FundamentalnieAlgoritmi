...
int list_pop_at(list* lst, unsigned long int position) {
  if (position >= list_size(lst)) {
    return ARGUMENTS_EXCEPTION;
  }
  if (position == 0) {
    return list_pop_front(lst);
  }
  list_node* lst_node = lst->head;
  for (unsigned long int i = 0; i < position - 1; ++i) {
    lst_node = lst_node->next;
...
int list_pop_back(list* lst) {
  return list_pop_at(lst, list_size(lst));
}
...
int list_clear(list* lst, void (*cls)(void* structure)) {
  ...
  while (!list_empty(lst)) {
    ...
    cls(elem);
    if ((execute_status = list_pop_front(lst)) != SUCCESS_FUNCTION_RETURN) {
      return execute_status;
    }
  }
  return SUCCESS_FUNCTION_RETURN;
