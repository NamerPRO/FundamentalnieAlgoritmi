...
nmemory_standards::sorted_list_memory::sorted_list_memory(void const * memory, size_t memory_size, allocate_type type, nlogger::logger * memory_logger): _memory_size(memory_size), _alloc_type(type), _was_allocated_via_constructor(memory == nullptr) {
...
void nmemory_standards::sorted_list_memory::log_memory(std::string const & message, nlogger::logger::severity level) const {
