...
//Cmp function return macro meanings:
//INCLUDE_CHARACTER = include symbol into result string
//SKIP_CHARACTER = just skip symbol; continue iterating
//STOP_ITERATION = end iteration process
//STOP_AND_INCLUDE = end iteration process and include symbol you stopped at
int read_string(FILE* stream, string* str, int (*cmp)(int symbol, int index), ...) {
  ...
  if (stream == FROM_STRING) {
    ...
    for (; symbol = str->str[*i]; ++*i) {
      ...
      if (cmp_result == INCLUDE_CHARACTER || cmp_result == STOP_AND_INCLUDE) {
        ...
        if (cmp_result == STOP_AND_INCLUDE) {
          break;
        }
      } else if (cmp_result == STOP_ITERATION) {
        break;
      }
    }
    ++*i;
