...
int list_pop_by(list* lst, string* s, void (*clear)(LIST_TYPE elem)) {
  ...
  if (!string_compare("%s", standart_string_comporator, s, prev_lst_node->data)) {
    return list_pop_front(lst, clear);
  }
  while (cur_lst_node != NULL && string_compare("%s", standart_string_comporator, s, cur_lst_node->data)) {
