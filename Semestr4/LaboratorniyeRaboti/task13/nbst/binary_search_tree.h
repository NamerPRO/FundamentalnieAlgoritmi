...
//        }
    ...
    template<
            typename tkey,
            typename tvalue,
            bool (*tkey_comporator)(tkey const &, tkey const &) = standard_comporators::basic_comporator
            >
        ...
        private:
            ...
            void insert_inner(
                ...
                if (_root_node == nullptr) {
                    // Создать метод get_size()
                    _root_node = ntools::nmalloc::malloc<tree_node *>(get_node_size()/*sizeof(tree_node)*/);

                    // Выов конструктора без выделения памяти
    ...
    //                find_template finder;
                    find_inner(key, _root_node, nodes_stack, false);

                    if (nodes_stack.top() != nullptr) {
                        on_insertable_element_exists(key, nodes_stack.top(), nodes_stack);
                    } else {
                        nodes_stack.pop();
    ...
    //                    tkey const & key,
        ...
        private:
            ...
            tvalue remove_inner(
                ...
                auto element_to_remove_data = remove_element_from_tree(nodes_stack);

                after_element_removed(key, element_to_remove_data.second.first, nodes_stack, element_to_remove_data.second.second);

                element_to_remove_data.second.first->~tree_node();
                ntools::nmalloc::free(element_to_remove_data.second.first);

                return element_to_remove_data.first;
            }
...
//            }
            ...
            virtual std::pair<tvalue, std::pair<tree_node *, tree_node *>> remove_element_from_tree(
                    std::stack<tree_node *> & nodes_stack
                    ) {
                tree_node * element_to_remove_ptr = nodes_stack.top();

                tree_node * ptr_to_return = nullptr;
                tree_node * instead_of_removed = nullptr;
                //            nodes_stack.pop();
...
//                    right_before_element_removed(key, nodes_stack);
                    ...
                    ptr_to_return = element_to_remove_ptr;
...
//                    right_before_element_removed(key, nodes_stack);
                    ...
                    instead_of_removed = next_node->right_child;
                    ptr_to_return = next_node;
                    // next_node->~tree_node();
...
//                    right_before_element_removed(key, nodes_stack);
                    ...
                    if (nodes_stack.empty()) {
                        ...
                        if (parent_node->left_child == element_to_remove_ptr) {
                            ...
                            instead_of_removed = parent_node->left_child;
                            // nodes_stack.push(parent_node->left_child);
                        } else {
                            if (element_to_remove_ptr->left_child != nullptr) {
                            ...
                            instead_of_removed = parent_node->right_child;
                            // nodes_stack.push(parent_node->right_child);
                        }
                    }
                    ptr_to_return = element_to_remove_ptr;
                    // element_to_remove_ptr->~tree_node();
                ...
                return { element_to_remove_data, { ptr_to_return, instead_of_removed } };
            }


            virtual void after_element_removed(
                    tkey const & key,
                    tree_node * element_removed_ptr,
                    std::stack<tree_node *> & nodes_stack,
                    tree_node * instead_of_removed_ptr
                    ) noexcept {
    ...
    //                    return node_to_return;
        ...
        protected:
            ...
            tree_node * rotate_left(
                ...
                tree_node * parent_node,
                std::stack<tree_node *> * nodes_stack = nullptr,
                size_t to_take = 0
            ) noexcept {
                ...
                if (_root_node == rotate_root) {
                    ...
                    if (parent_node->left_child == rotate_root) {
                        parent_node->left_child = saved_right_child;
                    } else {
                        parent_node->right_child = saved_right_child;
                    }
                }

                if (nodes_stack != nullptr) {
                    std::stack<tree_node *> to_take_stack;

                    while (to_take--) {
                        to_take_stack.push(nodes_stack->top());
                        nodes_stack->pop();
                    }
                    ...
                    nodes_stack->push(saved_right_child);
                    nodes_stack->push(rotate_root);

                    while (!to_take_stack.empty()) {
                        nodes_stack->push(to_take_stack.top());
                        to_take_stack.pop();
                    }
                }
            ...
            tree_node * rotate_right(
                ...
                if (_root_node == rotate_root) {
                    ...
                    if (parent_node->left_child == rotate_root) {
                        parent_node->left_child = saved_left_child;
                    } else {
                        parent_node->right_child = saved_left_child;
                    }
                }
                ...
                if (nodes_stack != nullptr) {
                    ...
                    nodes_stack->push(saved_left_child);
                    nodes_stack->push(rotate_root);
